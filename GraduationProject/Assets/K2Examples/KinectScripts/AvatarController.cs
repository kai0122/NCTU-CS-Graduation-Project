...
{
    public ServerCanvas m_ServerCanvas;
    ...
    public void UpdateAvatar(Int64 UserID)
    {
        ...
        if (gameObject.name == "U_CharacterFront(Clone)2")
        {
           //gameObject.transform.position = new Vector3(m_ServerCanvas.X, gameObject.transform.position.y, m_ServerCanvas.Z);
        }
    ...
    public bool ifLeftHandGrip, ifRightHandGrip;
    // Apply the rotations tracked by kinect to fingers (one joint = multiple bones)
    ...
    protected void MoveAvatar(Int64 UserID)
    {
        ...
        if (gameObject.transform.name == "U_CharacterFront(Clone)2")
            targetPos = new Vector3(m_ServerCanvas.playerPos.x, targetPos.y, m_ServerCanvas.playerPos.z);
        //Debug.Log(bodyRootPosition);
        ...
        if (gameObject.transform.name == "U_CharacterFront(Clone)2")
            gameObject.transform.position = new Vector3(m_ServerCanvas.playerPos.x, targetPos.y, m_ServerCanvas.playerPos.z);
    ...
    protected Vector3 Kinect2AvatarPos(Vector3 jointPosition, bool bMoveVertically)
    {
        ...
        m_ServerCanvas = GameObject.Find("Canvas").GetComponent<ServerCanvas>();
        cameraEye = GameObject.Find("Camera (eye)");
        Vector3 newPosition;
        if (gameObject.transform.name == "U_CharacterFront(Clone)2")
            newPosition = new Vector3(m_ServerCanvas.playerPos.x, bMoveVertically ? yPos : 0f, m_ServerCanvas.playerPos.z);
        else
        {
            newPosition = new Vector3(cameraEye.transform.position.x, bMoveVertically ? yPos : 0f, cameraEye.transform.position.z);
        }
